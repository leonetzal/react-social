...
class EditProfile extends Component {
    ...
    isValid = () => {
        ...
        if (fileSize > 1000000) {
            this.setState({ error: "File size should be less tan 1000kb" });
            return false;
        }
        if (name.length === 0) {
    ...
    clickSubmit = event => {
        ...
        if (this.isValid()) {
            const userId = this.props.match.params.userId;
            const token = isAuthenticated().token;
            update(userId, this.userData, token)
                .then(data => {
    ...
    render() {
        ...
        const photoUrl = id ? `${process.env.REACT_APP_API_URL}/user/photo/${id}?${new Date().getTime()}` : DefaultImageProfile;
        return (
            <div className="container" >
                ...
                <img style={{ height: "200px", width: 'auto' }} className="img-thumbnail" src={photoUrl} onError={i => (i.target.src = DefaultImageProfile)} alt={name} />
